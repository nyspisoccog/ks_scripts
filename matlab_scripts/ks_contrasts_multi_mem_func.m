        ...
        if strcmp(sess_type{lm}, 'lrn')
            ...
            for j = 1:numel(names)
                ...
                elseif isempty(strfind(names{j}, 'MSRT*bf(1)')) ~= 1 
                   Scount = Scount + 1;
                elseif isempty(strfind(names{j}, 'LYRT*bf(1)')) ~= 1
                   Ycount = Ycount + 1;
                end
            end
            convec = zeros(1, length(names));
            con_struct(1).name = 'SvY(R-I)'
