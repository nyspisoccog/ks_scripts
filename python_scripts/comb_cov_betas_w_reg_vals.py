...
for clust in clust_list:
    covb_fname = os.path.join(root, 'clust' + str(clust) + '.txt')
    ...
    att_trips = [('LYRS', 'LNRS', 'LYNRS'), ('LYRF', 'LNRF', 'LYNRF'),
                 ('LYIS', 'LNIS', 'LYNIS'), ('LYIF', 'LNIF', 'LYNIF'),
                 ('LSRS', 'LSRF', 'LSR'), ('LYRS', 'LYRF', 'LYR'),
    ...
    out.write(','.join(['regressor', 'cov_beta', 'avg_val']))
    out.write('\n')
    with open(covb_fname, 'r') as covb:
        for i, line in enumerate(csv.reader(covb)):
            if i > 0:
                ...
                out.write(','.join([reg_name, b, str(reg)]))
                out.write('\n')
