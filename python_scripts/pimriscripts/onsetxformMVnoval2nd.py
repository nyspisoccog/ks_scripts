...
#This version collects only the second half of learning trials.
...
header = ['subject', 'run', 'fullcond', 'type', 'rel', 'taught', 'onset', 'offset', 'RT', 'ans', 'BP']

with open(os.path.join(root, 'ks-trialsdata-wtrialtypename-noMV-noval-2ndhalf.csv'), 'w') as out:
    ...
    for sub in subs:
        ...
        for run in sub.runs:
            ...
            if 'L' in run.id:
                run.trials = run.trials[10:]
            for trial in run.trials:
                trial[4] = trial[4] - run.start
