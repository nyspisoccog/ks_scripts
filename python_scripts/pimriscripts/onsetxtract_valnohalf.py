...
#this version of onsetxtract_fullcond takes care of missing values
#includes valence, does not contrast first and second half
#it includes button press onsets
...
root = '/media/truecrypt1/SocCog/onsets/val_wbp'


f = open(os.path.join(root, 'ks-trialsdata-wtrialtypename-val.csv'), 'r')
last_run = ''
subruns = []
...
for subrun in subruns:
    ...
    out.close()
    bp_outpath = uf.buildtree(root, ['bp'])
    bp_out = open(os.path.join(root, 'bp', subrun.subid + subrun.runid + '.txt'), 'w')
    bp_out.write('\n'.join(subrun.bp))
    bp_out.close()
    for cond in subrun.conds:
