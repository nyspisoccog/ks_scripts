...
for clust in clust_files:
    ...
    with open(data_file, 'r') as data:
        for line in csv.DictReader(data):
            ...
            r.hi = float(line['predplus'])
            r.lo = float(line['predminus'])

    regs1 = ('LSRB-LSRAhi', 'LSRB-LSRAlow', 'LSIB-LSIAhi', 'LSIB-LSIAlo',
             'LYNRB-LYNRAhi','LYNRB-LYNRAlo', 'LYNIB-LYNIAhi','LYNIB-LYNIAlo' )
    ...
    for i, trip_list in enumerate(reg_trips):
        ...
        for j, trip in enumerate(trip_list):
            ...
            reg.min = min(s, m)
            reg.max = max(s, m)
            for v in [s, m, shi, slo, mlo, mhi]:
                if v < row.miny:
                    row.miny = v
                if v > row.maxy:
                    row.maxy = reg.max
            reg.dif = abs(s - m)
            reg.lodif = abs(slo - mlo)
            reg.hidif = abs(shi - mhi)
            reg.himin = min(shi, mhi)
            reg.lomin = min(slow, mlow)
            if s >= m:
                reg.mrk = '^'
            else:
                reg.mrk = 'v'
            ...
            #rect = mpatches.Rectangle([reg.pos - .1, reg.min], 0.2, reg.dif, ec="none",
                                  #color=reg.clr)
            lorect = mpatches.Rectangle([reg.pos - .1, reg.min], 0.2, reg.dif, ec="none",
                                  #color=reg.clr)
