...
for (i in 1:length(cluster_list)){
  ...
  for (j in 1:length(reg_list)){
    ...
    covplus1sd[j] = mean(reg[[j]]$V3) + sd(reg[[j]]$V3)
    covmnus1sd[j] = mean(reg[[j]]$V3) - sd(reg[[j]]$V3)
    ...
    predregplus[j] = b[[j]]*covplus1sd[[j]]
    predregmnus[j] = b[[j]]*covmnus1sd[[j]]
  }
  #what do I want?  I want a matrix with regressors as row names and a single column, b
  # that has 
  b_mat = cbind(reg_list, b, p, sig, predregplus, predregmnus)
