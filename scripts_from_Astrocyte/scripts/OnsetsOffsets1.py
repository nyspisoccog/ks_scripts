...
for line in f[1:]:
    ...
    if getRun(line)[0] != '':
        ...
        onset = int(onset)
        offset = int(offset)
        cond.events.append((onset, offset))
        run.events.append((onset, offset))
...
for sub in subs:
    ...
    for run in sub.sorted_runs:
        if len(run.events) > 0: run.start = sorted(run.events)[0][0]
        ...
        for cond in run.sorted_conds:
            ...
            for event in cond.sorted_events:
                sub_event = (event[0] - run.start, event[1] - run.start)
                cond.subtracted_events.append(sub_event)




header = ['subject', 'run', 'cond', 'onset', 'offset']
    ...
    for run in sub.sorted_runs:
        for cond in run.sorted_conds:
            for event in cond.subtracted_events:
                h.write("\t\t".join([sub.label, run.label, cond.label, str(event[0]), str(event[1])]) + '\n')
