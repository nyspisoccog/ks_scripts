...
for sub in subjects:
    ...
    for tags in soup8.find_all('tr'):
        ...
        if len(tag_list) > 1:
            e = exam_init(PPID=sub.PPID, number=tag_list[0].get_text(), date=tag_list[1].get_text(),\
                          study=tag_list[2].get_text(), unit=tag_list[3].get_text(), \
                          RA=tag_list[4].get_text(), num_series=tag_list[5].get_text(),\
                          notes=tag_list[9].get_text())
            sub.exams.append(e)
...
with open('/media/katie/SocCog/Scan_Record.csv', 'wb+') as csvfile:
...
with open('/media/katie/SocCog/Scan_Details.csv', 'wb+') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
